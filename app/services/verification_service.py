...
class VerificationService:
    ...
    async def verify_certificate(
        ...
        if extracted_hash:
            ...
            if unique_chars < settings.min_entropy_threshold:
                ...
                if expected_hash:
                    ...
                    print("[INFO] No expected hash provided for comparison.")
                    result['verification_status'] = VerificationStatus.UNKNOWN
                    result['message'] = "Hash extracted but no expected hash provided for verification"
        else:
        ...
        if cert_data and cert_data.get('Certificate Number'):
            ...
            # Additional verification through database matching
            if check_database and result['verification_status'] in [
            ...
            # Generate hash if not extracted
            if not result['hash']:
    ...
    async def _compare_certificate_data(
        ...
        for field in comparison_fields:
            if field in stored_data and field in extracted_data:
                total_fields += 1
                stored_value = str(stored_data[field]).strip().lower()
                extracted_value = str(extracted_data[field]).strip().lower()
